...
export function NewTenantPage() {
  ...
  const goBack = useSmartBack('/tenants');
  ...
  const mutation = useMutation({
    ...
    onError: (err: Error) => {
      if (
        ...
        err.message.includes('nric_passport')
      ) {
        form.setError('nric_passport', {
          message: 'This NRIC/Passport is already registered.',
        });
      } else {
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 lg:p-8">
      ...
      {/* Header */}
      <div className="mb-8">
        <BackNav to="/tenants" label="Back" className="mb-4" />
        <h1 className="text-2xl font-bold text-foreground">Add New Tenant</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Register a new tenant. A lease can be created after.
        </p>
      </div>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className="text-muted-foreground/50 text-xs">
                          Include country code: +60…
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
                    )}
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea
                        ...
                        className="bg-muted border-border text-foreground placeholder:text-muted-foreground/50 focus:border-violet-500/60 resize-y min-h-[72px]"
                        {...field}
                      />
                    </FormControl>
                  </FormItem>
