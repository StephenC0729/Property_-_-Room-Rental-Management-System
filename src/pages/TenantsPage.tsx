      ...
      <div className="group flex items-center gap-4 rounded-xl border border-white/6 bg-white/[0.02]
        ...
        <div className="shrink-0">
          {roomCode ? (
            ...
            <Badge className="bg-white/5 text-white/25 border-white/8 text-xs">No active room</Badge>
          )}
        </div>
    ...
    <div className="min-h-screen bg-[#0a0a0f] p-4 md:p-6 lg:p-8">
      ...
      <div className="relative mb-4 max-w-md">
        ...
        <Input
          ...
          className="pl-10 bg-white/5 border-white/10 text-white placeholder:text-white/20 focus:border-violet-500/50 h-10"
        />
      </div>
      ...
      {isLoading ? (
        <div className="space-y-2">
          {[...Array(6)].map((_, i) => <Skeleton key={i} className="h-16 rounded-xl bg-white/5" />)}
        </div>
      ) : !filtered.length ? (
        <Card className="border-white/8 bg-white/[0.03] p-12 text-center">
          <User className="mx-auto mb-4 h-12 w-12 text-white/15" />
          {search ? (
            <>
              <h3 className="text-base font-semibold text-white/40">No results for "{search}"</h3>
              <p className="mt-1 text-sm text-white/25">Try a different name, NRIC, or phone number.</p>
            </>
          ) : (
            <>
