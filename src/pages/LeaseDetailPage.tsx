...
export function LeaseDetailPage() {
  ...
  const utilitiesPaid =
    payments?.reduce((s, p) => s + getUtilitiesCollected(p), 0) ?? 0;
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 lg:p-8">
      ...
      {/* Back */}
      <BackNav to="/leases" label="Leases" className="mb-6" />
      ...
      <div className="max-w-3xl space-y-5">
        ...
        <div className="flex items-start justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-muted-foreground/70 flex items-center gap-1.5">
              ...
              {lease.rooms?.properties?.name ?? '—'} · Room{' '}
              {lease.rooms?.code ?? '—'}
            </p>
          </div>
        ...
        <Card className="border-border bg-card p-6 space-y-5">
          ...
          <InfoGrid
            items={[
              {
                label: 'Monthly Rent',
                value: formatRinggit(lease.monthly_rent),
                highlight: true,
              },
              { label: 'Due Day', value: `Day ${lease.due_day} of each month` },
              {
                label: 'Security Deposit',
                value: formatRinggit(lease.security_deposit),
              },
              {
                label: 'Utility Deposit',
                value: formatRinggit(lease.utility_deposit),
              },
            ]}
          />
          ...
          <InfoGrid
            items={[
              ...
              {
                label: 'Created',
                value: format(new Date(lease.created_at), 'dd MMM yyyy'),
              },
              {
                label: 'Duration',
                value:
                  lease.move_in_date && lease.expiry_date
                    ? `${differenceInDays(new Date(lease.expiry_date), new Date(lease.move_in_date))} days`
                    : '—',
              },
            ]}
          />
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          ...
          <Card className="border-border bg-card p-4">
            ...
            <Link
              to={`/properties/${lease.rooms?.property_id}`}
              className="group"
            >
              <p className="text-sm font-semibold text-foreground group-hover:text-violet-300 transition-colors">
                {lease.rooms?.properties?.name ?? '—'} · Room{' '}
              ...
              <p className="text-xs text-muted-foreground/70 mt-1">
                Room No. {lease.rooms?.room_number}
              </p>
              <p className="text-xs text-muted-foreground/70 mt-0.5">
                Base rent: {formatRinggit(lease.rooms?.base_rent ?? 0)}
              </p>
              <p className="text-xs text-violet-400 mt-2 group-hover:text-violet-300 transition-colors">
                View room matrix →
              </p>
            ...
            {paymentCount > 0 && (
              <div className="flex items-center gap-3 text-xs text-muted-foreground/70">
                ...
                <span className="text-emerald-400 font-medium">
                  {formatRinggit(rentPaid)} rent
                </span>
                {utilitiesPaid > 0 && (
                  <span className="text-sky-400 font-medium">
                    {formatRinggit(utilitiesPaid)} utilities
                  </span>
                )}
                <span className="text-violet-300 font-medium">
                  {formatRinggit(totalPaid)} total
                </span>
              </div>
            )}
          </div>
                  ...
                  >
                    <div>
                      <p className="text-foreground font-medium">
                        {format(
                          parseBillingMonthKey(payment.billing_month),
                          'MMMM yyyy',
                        )}
                      </p>
                      <p className="text-xs text-muted-foreground/70 mt-0.5">
                        Paid{' '}
                        {format(
                          new Date(payment.payment_date ?? payment.paid_at),
                          'dd MMM yyyy',
                        )}
                      </p>
                    </div>
                    ...
                    <div className="text-right">
                      ...
                      {utilities > 0 && (
                        <p className="text-[11px] text-muted-foreground/70 mt-0.5">
                          {formatRinggit(payment.amount)} rent ·{' '}
                          {formatRinggit(utilities)} utilities
                        </p>
                      )}
                    </div>
                    {isAdmin() && (
