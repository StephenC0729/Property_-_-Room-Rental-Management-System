  ...
  switch (action) {
    ...
    case 'LEASE_UPDATED':
      return [
        meta.room_code ? `Room ${meta.room_code}` : null,
        meta.monthly_rent
          ? `RM ${Number(meta.monthly_rent).toFixed(2)}/mo`
          : null,
      ]
        .filter(Boolean)
        .join(' · ');
...
function useAuditLog(
  ...
  return useQuery({
    queryKey: ['audit-log', dateRange, actionFilter, page],
    queryFn: async () => {
      ...
      const { data: entries, error: entriesError, count } = await q;
      if (entriesError) throw entriesError;
      if (!entries?.length) {
        return { entries: [] as AuditEntry[], totalCount: count ?? 0 };
      }
  ...
  return (
    <div className="flex gap-4">
      ...
      <div className="flex flex-col items-center shrink-0">
        <div
          className={`flex h-8 w-8 items-center justify-center rounded-full ${cfg.bgColor} shrink-0`}
        >
          <Icon className={`h-3.5 w-3.5 ${cfg.color}`} />
        </div>
        {!isLast && (
          <div className="mt-1 w-px flex-1 min-h-[16px] bg-white/6" />
        )}
      </div>
...
export function AuditLogPage() {
  ...
  const {
    ...
    error,
    refetch,
  } = useAuditLog(dateRange, actionFilter, page);
  ...
  // Client-side search (target_id or metadata values) within the current page
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {isLoading
              ? '—'
              : totalCount === 0
                ? '0 events · auto-refreshes every 30s'
                : search.trim() && filtered.length !== (entries?.length ?? 0)
                  ? `${filtered.length} matching on this page · ${totalCount} total · auto-refreshes every 30s`
                  : `${pageStart}–${pageEnd} of ${totalCount} events · auto-refreshes every 30s`}
          </p>
        </div>
        <Button
          variant="ghost"
      ...
      {isLoading ? (
        ...
        <Card className="border-border bg-card p-12 text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground/50">
            {search
              ? 'No matching events for your search.'
          ...
          {grouped.map((group) => (
            <div key={group.date}>
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="text-xs font-medium text-muted-foreground/50 px-2">
                  {group.date}
                </span>
                <div className="h-px flex-1 bg-white/6" />
              </div>
