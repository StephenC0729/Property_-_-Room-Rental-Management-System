...
import {
  tenantSchema as editSchema,
  type TenantFormValues as EditFormValues,
} from '@/schemas/tenant';
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input
                  className="bg-muted border-border text-foreground focus:border-violet-500/60 h-9"
                  {...field}
                />
              </FormControl>
              <FormMessage />
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea
                  rows={3}
                  className="bg-muted border-border text-foreground focus:border-violet-500/60 resize-y min-h-[72px]"
                  {...field}
                />
              </FormControl>
            </FormItem>
...
function LeaseCard({ lease }: { lease: LeaseWithRoom }) {
  ...
  const isExpired =
    !isActive && lease.expiry_date && isPast(new Date(lease.expiry_date));
  const room = lease.rooms;
  const statusBadge = getLeaseStatusBadge(lease.status);

  return (
    <div
      className={`rounded-xl border p-4 space-y-3 ${isActive ? 'border-emerald-500/20 bg-emerald-500/5' : 'border-white/6 bg-card'}`}
    >
      <div className="flex items-start justify-between">
        ...
        <Badge className={`text-xs ${statusBadge.cls}`}>
          {statusBadge.label}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-xs">
        ...
        <div>
          ...
          <p className="font-semibold text-foreground">
            {lease.move_in_date
              ? format(new Date(lease.move_in_date), 'dd MMM yyyy')
              : '—'}
          </p>
        </div>
        <div>
          ...
          <p
            className={`font-semibold ${isExpired && isActive ? 'text-red-400' : 'text-foreground'}`}
          >
            {lease.expiry_date
        ...
        <div className="flex gap-4 text-xs border-t border-white/5 pt-3">
          <span className="text-muted-foreground/70">
            ...
            <span className="text-muted-foreground">
              {formatRinggit(lease.security_deposit)}
            </span>
          </span>
          <span className="text-muted-foreground/70">
            Utility deposit:{' '}
            <span className="text-muted-foreground">
              {formatRinggit(lease.utility_deposit)}
            </span>
          </span>
        </div>
      )}
      ...
      {isActive && (
        <div className="pt-1">
          <Button
            ...
            size="sm"
            variant="ghost"
            className="h-7 text-xs text-violet-400 hover:text-violet-300 px-0"
          >
    ...
    <div
      className={
        multiline
          ...
          : 'flex items-center justify-between py-2.5 border-b border-white/5 last:border-0'
      }
    >
      ...
      <span
        className={
          multiline
            ? 'text-sm text-foreground font-medium mt-1 whitespace-pre-wrap'
            : 'text-sm text-foreground font-medium text-right max-w-[60%] truncate'
        }
      >
        {value ?? '—'}
      </span>
...
export function TenantProfilePage() {
  ...
  return (
    <div className="min-h-screen bg-background p-4 md:p-6 lg:p-8">
      ...
      {/* Back */}
      <BackNav to="/tenants" label="Tenants" className="mb-6" />
          ...
          <div className="flex items-center gap-3 rounded-xl border border-emerald-500/20 bg-emerald-500/8 px-4 py-3">
            ...
            <p className="text-sm text-emerald-300">
              ...
              <span className="font-semibold">
                Room {activeLease.rooms?.code}
              </span>{' '}
              — {activeLease.rooms?.properties?.name} ·{' '}
              {formatRinggit(activeLease.monthly_rent)}/month
            </p>
            <Button
              asChild
              size="sm"
              variant="ghost"
              className="ml-auto text-emerald-400 hover:text-emerald-300 text-xs h-7"
            >
          ...
          {isEditing ? (
            ...
            <>
              <div className="mb-2">
                ...
                <InfoRow
                  label="NRIC / Passport"
                  value={tenant.nric_passport}
                  icon={CreditCard}
                />
                <InfoRow label="Phone" value={tenant.phone} icon={Phone} />
                {tenant.notes && (
                  <InfoRow label="Notes" value={tenant.notes} multiline />
                )}
              </div>
              ...
              {(tenant.emergency_name || tenant.emergency_phone) && (
                <>
                  ...
                  <InfoRow
                    label="Relation"
                    value={tenant.emergency_relation ?? null}
                  />
                  <InfoRow
                    label="Phone"
                    value={tenant.emergency_phone ?? null}
                  />
                </>
              )}
            </>
        ...
        <div>
          ...
          {!tenant.leases?.length ? (
            ...
            <div className="space-y-3">
              {activeLease && (
                <LeaseCard key={activeLease.id} lease={activeLease} />
              )}
              {pastLeases.map((l) => (
